 ...
 * @Last Modified time: 2019-03-13 11:13:40
 * @Func 用户及其权限控制的业务逻辑
 */

const Service = require('egg').Service;
const {getStudentInfo} = require('../../sql/exam');
class UserService extends Service{
    ...
    async createUser(userInfo){
        ...
        let result = await this.app.mysql.insert('user',{...userInfo,user_id});
        return {row:result.affectedRows,user_id};
    }
    // 更新用户(可为用户设置身份)
    async updateUser(user_id,willUpdated){
        if(willUpdated.user_name){
            let isRepeat = await this.isRepeat('user','user_name',willUpdated.user_name);
            if(isRepeat){
                return 'repeat';
            }
        }
        let result = await this.app.mysql.update('user',willUpdated,{where:{user_id}});
    ...
    async addIdentity(identity_text){
        ...
        let result = await this.app.mysql.insert('identity',{identity_text,identity_id});
        return {row:result.affectedRows,identity_id};
    }
    // 添加视图权限
    async addAuthorityView(info){
        let {view_authority_text,view_id} = info;
        ...
        let result = await this.app.mysql.insert('view_authority',{...info,view_authority_id});
        return {row:result.affectedRows,view_authority_id};
    }
    // 添加api接口权限
    async addAuthorityApi(info){
        let {api_authority_text,api_authority_url,api_authority_method} = info;
        let textRepeat = await this.isRepeat('api_authority','api_authority_text',api_authority_text);
        if(textRepeat){
            return 'repeat';
        }
        let urlResult = await this.app.mysql.select('api_authority',{where:{api_authority_url,api_authority_method}});
        if(urlResult.length > 0){
            return 'repeat';
        }
        let api_authority_id = this.ctx.helper.randomString(2);
        let result = await this.app.mysql.insert('api_authority',{...info,api_authority_id});
        return {row:result.affectedRows,api_authority_id};
    }
    // 为身份设定视图权限
    async addIdentityView(info){
        let {identity_id,view_authority_id} = info;
        let viewResult = await this.app.mysql.select('identity_view_authority_relation',{where:{identity_id,view_authority_id}});
        if(viewResult.length > 0){
            return 'repeat';
        }
        let identity_view_authority_relation_id = this.ctx.helper.randomString(3);
        let result = await this.app.mysql.insert('identity_view_authority_relation',{...info,identity_view_authority_relation_id});
        return result.affectedRows;
    }
    // 为身份设定api权限
    async addIdentityApi(info){
        let {identity_id,api_authority_id} = info;
        let apiResult = await this.app.mysql.select('identity_api_authority_relation',{where:{identity_id,api_authority_id}});
        if(apiResult.length > 0){
            return 'repeat';
        }
        let identity_api_authority_relation_id = this.ctx.helper.randomString(3);
        let result = await this.app.mysql.insert('identity_api_authority_relation',{...info,identity_api_authority_relation_id});
        return result.affectedRows;
    }
    // 根据用户的id和api接口判断用户是否具有该权限
    async isUserApiAuthority({url,method,user_id}){
